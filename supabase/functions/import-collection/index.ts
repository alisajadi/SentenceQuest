...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (json?.format !== "sentencequest.collection") {
      throw new Error("Invalid collection format.");
    }

    const languageData = json.language;
    const collectionData = json.collection;
    ...
    for (const item of json.vocabulary ?? []) {
        ...
        .upsert(
          {
            language_id: language.id,
            word: item.word,
            normalized_word: normalizedWord(item.word),
      ...
      vocabularyMap.set(item.id, vocabulary.id);
      ...
      if (
        item.forms &&
        typeof item.forms === "object"
      ) {
        for (
          const [formType, formValue]
          of Object.entries(item.forms)
        ) {
          const {
            error: formError,
          } = await db
    ...
    for (const item of json.grammar ?? []) {
      const {
        data: grammar,
        error,
      } = await db
    ...
    for (const item of json.patterns ?? []) {
      const grammarId = grammarMap.get(
        item.grammar_code ??
          json.grammar?.[0]?.code
      );
    ...
    for (const item of json.lessons ?? []) {
      ...
      const {
        error: deleteLessonWordsError,
      } = await db
        .from("lesson_words")
        .delete()
        .eq("lesson_id", lesson.id);

      if (deleteLessonWordsError) {
      ...
      if (item.grammar_intro) {
        ...
        const introData = {
          ...
          short_text:
            intro.short_text ?? null,

          detailed_text:
            intro.detailed_text ?? null,

          image_url:
            intro.image_url ?? null,

          audio_url:
            intro.audio_url ?? null,

          audio_duration_seconds:
            intro.audio_duration_seconds ?? null,

          video_url:
            intro.video_url ?? null,

          video_duration_seconds:
            intro.video_duration_seconds ?? null,
